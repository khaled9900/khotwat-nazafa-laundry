...
const EmployeeDepartments = () => {
  ...
  const fetch = async () => {
    const { data } = await supabase.from("employee_departments").select("*").order("name");
  ...
  const handleSave = async () => {
    if (!name.trim()) return;
    if (editId) {
      await supabase.from("employee_departments").update({ name }).eq("id", editId);
      toast({ title: "تم التحديث" });
    } else {
      await supabase.from("employee_departments").insert({ name });
  ...
  const handleDelete = async (id: string) => {
    ...
    await supabase.from("employee_departments").delete().eq("id", id);
    ...
    <div className="flex-1 overflow-y-auto scrollbar-thin bg-background">
      <div className="max-w-3xl mx-auto p-6 space-y-6">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-foreground">أقسام الموظفين</h1>
        </div>
        <div className="flex gap-2">
          <Input placeholder="اسم القسم..." value={name} onChange={(e) => setName(e.target.value)} className="flex-1" />
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            ...
            {items.length === 0 && <div className="text-center py-8 text-muted-foreground">لا توجد أقسام</div>}
          </div>
        )}
      </div>
    </div>
