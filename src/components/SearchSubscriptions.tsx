import { useState, useEffect } from "react";
import { supabase } from "@/integrations/supabase/client";
import { CreditCard, Search } from "lucide-react";
import { Input } from "@/components/ui/input";

const SearchSubscriptions = () => {
  const [customers, setCustomers] = useState<any[]>([]);
  const [search, setSearch] = useState("");
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    (async () => {
      const { data } = await supabase.from("customers").select("*").order("total_spent", { ascending: false });
      if (data) setCustomers(data);
      setLoading(false);
    })();
  }, []);

  const filtered = customers.filter((c) => c.name.includes(search) || c.phone?.includes(search));
  const totalSpent = filtered.reduce((s, c) => s + Number(c.total_spent), 0);
  const totalOrders = filtered.reduce((s, c) => s + c.total_orders, 0);

  return (
    <div className="flex-1 overflow-y-auto scrollbar-thin bg-background">
      <div className="max-w-5xl mx-auto p-6 space-y-6">
        <div className="flex items-center gap-2">
          <CreditCard className="h-6 w-6 text-primary" />
          <h1 className="text-xl font-bold text-foreground">الحساب العام للإشتراكات</h1>
        </div>

        <div className="relative">
          <Search className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
          <Input placeholder="بحث بالاسم أو الهاتف..." value={search} onChange={(e) => setSearch(e.target.value)} className="pr-10" />
        </div>

        <div className="grid grid-cols-3 gap-4">
          <div className="bg-card rounded-lg border border-border p-4 text-center">
            <p className="text-sm text-muted-foreground">عدد العملاء</p>
            <p className="text-2xl font-extrabold text-foreground">{filtered.length}</p>
          </div>
          <div className="bg-card rounded-lg border border-border p-4 text-center">
            <p className="text-sm text-muted-foreground">إجمالي الطلبات</p>
            <p className="text-2xl font-extrabold text-primary">{totalOrders}</p>
          </div>
          <div className="bg-card rounded-lg border border-border p-4 text-center">
            <p className="text-sm text-muted-foreground">إجمالي الإنفاق</p>
            <p className="text-2xl font-extrabold text-accent">{totalSpent.toLocaleString()} ر.س</p>
          </div>
        </div>

        {loading ? (
          <div className="text-center py-12 text-muted-foreground">جاري التحميل...</div>
        ) : (
          <div className="bg-card rounded-lg border border-border overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-secondary/30">
                  <th className="text-right py-3 px-4 font-semibold text-foreground">#</th>
                  <th className="text-right py-3 px-4 font-semibold text-foreground">العميل</th>
                  <th className="text-right py-3 px-4 font-semibold text-foreground">الهاتف</th>
                  <th className="text-center py-3 px-4 font-semibold text-foreground">الطلبات</th>
                  <th className="text-right py-3 px-4 font-semibold text-foreground">الإنفاق</th>
                  <th className="text-right py-3 px-4 font-semibold text-foreground">المتوسط</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((c, i) => (
                  <tr key={c.id} className="border-b border-border/50 hover:bg-secondary/20">
                    <td className="py-3 px-4 text-muted-foreground">{i + 1}</td>
                    <td className="py-3 px-4 font-medium text-foreground">{c.name}</td>
                    <td className="py-3 px-4 text-muted-foreground">{c.phone || "—"}</td>
                    <td className="py-3 px-4 text-center">{c.total_orders}</td>
                    <td className="py-3 px-4 font-bold">{Number(c.total_spent).toLocaleString()} ر.س</td>
                    <td className="py-3 px-4 text-muted-foreground">{c.total_orders > 0 ? Math.round(Number(c.total_spent) / c.total_orders).toLocaleString() : 0} ر.س</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        )}
      </div>
    </div>
  );
};

export default SearchSubscriptions;
